...
function ResumeHistory() {
  ...
  useEffect(() => {
    ...
    if (filter === "70") {
      filtered = filtered.filter((resume) => resume.atsScore >= 70);
    }

    if (filter === "80") {
      filtered = filtered.filter((resume) => resume.atsScore >= 80);
    }

    if (filter === "90") {
      filtered = filtered.filter((resume) => resume.atsScore >= 90);
    }
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      ...
      {filteredResumes.length === 0 ? (
        <div className="bg-white rounded-xl shadow-lg p-8">
          <h2 className="text-center text-xl">No Resume Found</h2>
        </div>
      ) : (
