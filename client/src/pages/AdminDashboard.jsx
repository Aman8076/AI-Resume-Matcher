...
function AdminDashboard() {
  ...
  const filteredResumes = useMemo(() => {
    return resumes.filter((resume) =>
      resume.filename.toLowerCase().includes(searchResume.toLowerCase()),
    );
  }, [resumes, searchResume]);

  const barData = {
    labels: ["Users", "Resumes"],

    datasets: [
      {
        label: "Platform Data",

        data: [stats.totalUsers, stats.totalResumes],
      },
    ],
  };
  ...
  const doughnutData = {
    ...
    datasets: [
      {
        data: [stats.averageATS, 100 - stats.averageATS],
      },
    ],
  };
